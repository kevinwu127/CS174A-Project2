  ...
  { 'construct'( context )
    ...
    'drawBee' ( model_transform, graphics_state ) {
      ...
      'drawStair' (model_transform, graphics_state, stair_num) {
        var stair = model_transform;

        omega = 0.1;
        const STAIR_LENGTH = 2;
        const STAIR_HEIGHT = 2;

        time = graphics_state.animation_time/1000;
        phi = stair_num;
        console.log(phi);
        stair = mult(stair, translation(0,Math.sin((time*2)+phi)*0.1,0));

        stair = mult(stair, translation(stair_num * STAIR_HEIGHT, stair_num * STAIR_LENGTH, 0));
        this.shapes.box.draw(graphics_state, stair, this.pink_clay);
    ...
    'drawStaircase' (model_transform, graphics_state) {
      var layer = model_transform;
