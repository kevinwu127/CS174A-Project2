...
Declare_Any_Class( "MelMel",    // A square, demonstrating shared vertices.  On any planar surface, the interior edges don't make any important seams.
  { 'populate'()                // In these cases there's no reason not to re-use values of the common vertices between triangles.  This makes all the
      {                         // vertex arrays (position, normals, etc) smaller and more cache friendly.
        this.positions     .push( vec3(-1,-1,0), vec3(1,1,0), vec3(-2,0,0), vec3(2,0,0) ); // Specify the 4 vertices -- the point cloud that our Square needs.
        this.normals       .push( vec3(0,1,0), vec3(0,-1,0), vec3(1,1,0), vec3(-1,1,0) );     // ...
        this.texture_coords.push( vec2(0,0),   vec2(1,0),   vec2(0,1),   vec2(1,1)   );     // ...
